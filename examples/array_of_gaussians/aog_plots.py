...
# Plot sequence of simple trajectories
import matplotlib.animation as animation

dt = 0.005
snapshot_times = np.arange(0, 10 + dt, dt)
...
def update(frame):
    ...
    contour = ax.contourf(X, Y, Z, levels=50, cmap='gray', vmin=0, vmax=10)  # Set background to grayscale
    ax.set_title(f'Simple Trajectories up to Time t = {t:.03f}')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    # plt.colorbar(contour, ax=ax, label='Potential Value')
    
    for i in range(2 * Nparticles):
        idx = (np.abs(simple_trajectories_times[i] - t)).argmin()  # Find the index for each particle
        ax.scatter(simple_trajectories_x[i][idx, 0], 
                   simple_trajectories_x[i][idx, 1], 
                   s=10, color='red', label=f'Particle {i+1}')

ani = animation.FuncAnimation(fig, update, frames=len(snapshot_times), interval=50)

# Save the animation as a video file
ani.save('simple_trajectories.mp4', writer='ffmpeg', fps=40)
...
dt = 0.005
snapshot_times = np.arange(alltime[0], 10 + dt, dt)
...
def update(frame):
    ax.clear()
    t = snapshot_times[frame]
    idx = (np.abs(alltime - t)).argmin()
    
    contour = ax.contourf(X, Y, Z, levels=50, cmap='gray', vmin=0, vmax=10)
    ax.set_title(f'Particle Trajectories up to Time t = {t:.03f}')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    
    for i in range(Nparticles):
        ax.scatter(allpositionsx[idx, i, 0], allpositionsx[idx, i, 1], color='red', s=10)
        ax.scatter(allpositionsy[idx, i, 0], allpositionsy[idx, i, 1], color='red', s=10)

ani = animation.FuncAnimation(fig, update, frames=len(snapshot_times), interval=50)

# Save the animation as a video file
ani.save('particle_trajectories.mp4', writer='ffmpeg', fps=40)
